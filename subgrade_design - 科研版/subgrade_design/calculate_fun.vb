...
Class calculate_fun
    ...
    '动荷载显著影响深度hh
    Shared Function hh#(v!, c!, p_out_max#(), q_g!, gamma_m!, p!) '轴重p的输入单位为t
        '割线法求动静应力比0.2的深度（含道床厚度）
        Dim f = Function(z#)
    ...
    Shared Function sigma#(z#, p_out_max#()， p!)
        ...
        '找到最大荷载分担比处的轨枕编号
        For i = Ceiling(p_out_max.Length / 2) To 0 Step -1 '从中间和向左逐个搜索（因为4Z1800的对称特性，单边搜索可靠），保证计算的应力最大
        ...
        For i = 0 To 1
            For j = 0 To p_out_max.Length - 1
                If j = n_max And i = 0 Then
                    temp = Boussinesq2(p_out_max(j) * (p / 2) / (1090 * 320 / 1000000.0), 545 / 160, z / 0.16, z) * 4 '均布力
                Else
                    temp = Boussinesq1(p_out_max(j) * (p / 2), Sqrt((i * 1.51) ^ 2 + ((j - n_max) * 0.6) ^ 2 + z ^ 2), z)  '集中力
                End If
                result = result + temp
            Next j
        Next i
    ...
    Shared Function Boussinesq2(p#, m#, n#, z#)
        Return p / 2 / PI * (
            m * n * (1 + m * m + 2 * n * n) / Sqrt(1 + m * m + n * n) / (1 + n * n) / (m * m + n * n) _
            + Atan(m / n / Sqrt(1 + m * m + n * n)))
    End Function
    ...
    Shared Function cal(n%, p#, p0#) As Double()
        ...
        Return p_out
    End Function
    Shared Function cal_1() As Double()
        Dim p_out = New Double() {0.0924, 0.2414, 0.3324, 0.2414, 0.0924}
        Return p_out
    End Function
