...
{
	vector <int> vec_nums;
	vec_nums.push_back(25);
	vec_nums.push_back(101);
	vec_nums.push_back(2011);
	vec_nums.push_back(-50);

	auto even_num_i = find_if(vec_nums.cbegin()
		, vec_nums.cend()
		, [](const size_t& num) { return((num % 2) == 0); });

	if (even_num_i != vec_nums.cend())
		cout << "Even number in collection is: " << *even_num_i << endl;
}
...
void example4()
{
	...
	transform(vec_multiplicand.cbegin()
		...
		, vec_result.begin()
		, [](const int& num1, const int& num2)->int {return (num1*num2); });
