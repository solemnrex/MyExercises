...
//				sprintf_s(buffer, "Uniform\\uniform%d_%d", j, iter + 1);
				// Our populations
				if (j < 7)
					sprintf_s(buffer, "DataValMax10000\\ArtSet%d_200000_%d_10000.txt", j, i);
				else
					sprintf_s(buffer, "DataValMax10000\\ArtSet%d_100000_%d.txt", j, i);
	...
	for (j = 2; j <= 6; j++) {
		...
		for (iter = 0; iter < 1; iter++) {
			...
			for (i = 1; i <= 5; i++) {
				...
				runningTime = clock() - t0;
				fstream stream("res.txt", ios::out | ios::ate | ios::app);
				streamStr << iter << '\t' << "NDTree\t" << j << "\t" << i << "\t" << treeSet.numberOfSolutions() << "\t" << runningTime << "\t" << "\n";
				cout << streamStr.str();
				stream << streamStr.str();
				streamStr.str("");
				stream.close();
				...
				runningTime = clock() - t0;
				fstream stream2("res.txt", ios::out | ios::ate | ios::app);
				streamStr << iter << '\t' << "List\t" << j << "\t" << i << "\t" << listSet.size() << "\t" << runningTime << "\t" << "\n";
				cout << streamStr.str();
				stream2 << streamStr.str();
				streamStr.str("");
				stream2.close();
				//				cout << pointComparisons << '\n';
				...
				if (j == 2) {
					...
					runningTime = clock() - t0;
					fstream stream2("res.txt", ios::out | ios::ate | ios::app);
					streamStr << iter << '\t' << "Sorted list\t" << j << "\t" << i << "\t" << listSetSorted.size() << "\t" << runningTime << "\n";
					cout << streamStr.str();
					stream2 << streamStr.str();
					streamStr.str("");
					stream2.close();
...
void ndTestClustred() {
	...
	for (j = 2; j <= 6; j++) {
		...
		for (iter = 0; iter < 10; iter++) {
			...
			int i;
			for (i = 1; i <= 5; i++) {
				sprintf_s(buffer, "ClusteredSets100\\ArtSetClust%d_200000_%d.txt", j, i);
				TListSet <TPoint> allSolutions;
				allSolutions.Load(buffer);
				cout << allSolutions.size() << '\n';
...
void ndTestNotUniform() {
	...
	for (j = 2; j <= 6; j++) {
		...
		for (iter = 0; iter < 1; iter++) {
			...
			int i;
			for (i = 1; i <= 5; i++) {
				sprintf_s(buffer, "NotUniform\\ArtSetNotUniform%d_200000_%d.txt", j, i);
				TListSet <TPoint> allSolutions;
				allSolutions.Load(buffer);
				cout << allSolutions.size() << '\n';
...
void ndTestFinalPLS() {
	...
	for (j = 2; j <= 6; j++) {
		Problem.setArtificialProblem(j);
		sprintf_s(buffer, "DataPLS150\\Pop100000_150_%d.txt", j);
		TListSet <TPoint> allSolutions;
		allSolutions.Load(buffer);
		cout << allSolutions.size() << '\n';
...
void ndTestFinalMOEAD() {
	...
	for (j = 2; j <= 6; j++) {
		Problem.setArtificialProblem(j);
		sprintf_s(buffer, "DataMOEAD\\PointsG_MOEAD_KS500%d_R1.DAT", j);
		TListSet <TPoint> allSolutions;
		allSolutions.Load(buffer);
		cout << allSolutions.size() << '\n';
